...
geneLoci$chromosome_name <- sapply(strsplit(cosmicGeneCensusv99$Genome.Location, ":"), function(x) x[1])
geneLoci$start_position <- sapply(strsplit(sapply(strsplit(cosmicGeneCensusv99$Genome.Location, "-"), function(x) x[1]), ":"), function(x) x[2])   
geneLoci$end_position <- sapply(strsplit(cosmicGeneCensusv99$Genome.Location, "-"), function(x) x[2])
...
desiredSamples <- desiredSamples[which(desiredSamples$ONCOTREE_CODE %in% c("PRAD")), ]
...
ampEvents <- data.frame(start = c("chr3-355", "chr14-45", "chr8-221", "chrX-176"),
                        end = c("chr3-359", "chr14-46", "chr8-223", "chrX-186"),
                        target = c("GATA2/3q", "FOXA1/14q", "MYC/8q", "AR/Xq"))


delEvents <- data.frame(start = c("chr8-25", "chr10-115", "chr13-146", "chr17-20"),
                        end = c("chr8-27", "chr10-117", "chr13-151", "chr17-23"),
                        target = c("NKX3-1/8p", "PTEN/10q", "RB1/13q", "TP53/17p"))
...
sampleSVs <- sv[which(sv$Sample_Id %in% desiredSamples$SAMPLE_ID), ]

# two event definitions:
# 1. classic ETS fusion: 5' - TMPRSS2 - ERG - 3'
# 2. any other fusion involving one of: ERG, ETV1, ETV4, ETV5

fusEvents <- data.frame(`TMPRSS2-ERG (Fus)` = rep(0, nrow(mutEvents)),
                        `x-ETS-x (Fus)` = rep(0, nrow(mutEvents)),
                        check.names = F)
rownames(fusEvents) <- rownames(mutEvents)

for (e in 1:nrow(sampleSVs)) {
  
  if (sampleSVs$Site1_Hugo_Symbol[e] == "TMPRSS2") {
    if (sampleSVs$Site2_Hugo_Symbol[e] == "ERG") {
      fusEvents[sampleSVs$Sample_Id[e], "TMPRSS2-ERG (Fus)"] <- 1
      next
    } else if (sampleSVs$Site2_Hugo_Symbol[e] %in% c("ETV1", "ETV4", "ETV5")) {
      fusEvents[sampleSVs$Sample_Id[e], "x-ETS-x (Fus)"] <- 1
      next
    }
  } else if (sampleSVs$Site1_Hugo_Symbol[e] %in% c("ERG", "ETV1", "ETV4", "ETV5") | sampleSVs$Site2_Hugo_Symbol[e] %in% c("ERG", "ETV1", "ETV4", "ETV5")) {
    fusEvents[sampleSVs$Sample_Id[e], "x-ETS-x (Fus)"] <- 1
  }
...
# choose fusion events
manualFusionEventIndices <- 1:2

# set how many muts are chosen based on frequency alone
mcMuts <- 17

# select those events plus the manually selected ones
select <- cbind(cnEventRecord[, c(0, manualCnEventsIndices)], mutEvents[, (0:mcMuts)], fusEvents[, c(0, manualFusionEventIndices)])
...
# omitHyperSamples <- rownames(pmForm[which(rowSums(pmForm[, colnames(pmForm)[which(colnames(pmForm) != "paired")]]) > 25), ])
...
# save outputs
write.csv(pmForm, paste0(getwd(), "/outputs/eventProjects/", projName, "/", projName, "_Events.csv"), row.names = T, quote = F)
write.csv(selectionRecord, paste0(getwd(), "/outputs/eventProjects/", projName, "/", projName, "_sampleSelection.csv"), row.names = F, quote = F)
